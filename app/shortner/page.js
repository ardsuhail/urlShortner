    ...
    fetch("/api/generate", requestOptions)
      ...
      .then((result) => {
        if (result.success) {
          ...
          const existingLinks = JSON.parse(sessionStorage.getItem("links")) || [];
          existingLinks.push(newLink);
          sessionStorage.setItem("links", JSON.stringify(existingLinks));
      ...
      >
        ...
        >
          ...
          <motion.button
            ...
            onClick={Generate}
            disabled={loading}
            className={`mt-6 w-full py-3 cursor-pointer rounded-xl text-white font-semibold shadow-lg transition-all duration-300 flex items-center justify-center gap-2 ${
              loading
